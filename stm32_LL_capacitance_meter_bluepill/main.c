...
//	  LL_mDelay(500);
	  ...
	  LL_TIM_EnableCounter(TIM1);
		LL_ADC_REG_StartConversionSWStart(ADC1);	      // start A/D conversion
	  }
	  ...
	  else if (flag==1)
	  {
	  	...
	  	if (LL_ADC_IsActiveFlag_EOS(ADC1)){
	  		 LL_ADC_ClearFlag_EOS(ADC1);
	  			     	 AD_last = LL_ADC_REG_ReadConversionData12(ADC1);
	                               /* use upper 12 bits of ADC  */
	  			     	 sprintf(string2,"the adc value is %i\n",AD_last);
	  			     	 usart_sendstring(string2);
	  	            if (AD_last>2580)
	  				{
	  					LL_ADC_REG_StopConversionExtTrig(ADC1); /* stop A/D conversion     */
	  	                flag=0;
	    	            LL_TIM_DisableCounter(TIM1); // counter enable
	  	                counter= LL_TIM_GetCounter(TIM1);
